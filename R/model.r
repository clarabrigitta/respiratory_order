...
### precompute fortnight lookup vector to avoid dplyr filter inside ODE (called at every timestep)
...
### example plot of immunity function
example_df <- data.frame(a = seq(1, 9, by = 1)) %>% 
  mutate(prob_susceptible = imm_distribution(a = a, mu = 3, sigma = 20),
         prob_immunity = 1 - prob_susceptible)

ggplot(example_df, aes(x = a, y = prob_susceptible)) +
  geom_line(linewidth = 1) +
  labs(x = "Age Groups", y = "Probability of immunity") +
  scale_x_continuous(breaks = seq(1, 9, 1), labels = c("0-4", "5-11", "12-17", "18-29", "30-39", "40-49", "50-59", "60-69", "70+")) +
  theme_bw() +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))

### define daily births and death rate
daily_births <- scot_births %>% select(births_daily) %>% pull() %>% mean() %>% floor()
...
## model ODEs ---- (vectorised: removes with(as.list()), uses matrix multiply for lambda)
seirs <- function(t, y, parms) {
  sigma <- parms$sigma
  gamma <- parms$gamma
  omega <- parms$omega
  ...
  # extract compartments as length-9 vectors (y ordered: S1,E1,I1,R1, S2,E2,I2,R2, ...)
  S <- y[seq(1, 33, by = 4)]
  E <- y[seq(2, 34, by = 4)]
  I <- y[seq(3, 35, by = 4)]
  R <- y[seq(4, 36, by = 4)]
  ...
  # force of infection via matrix multiply (replaces 81 scalar operations)
  contacts <- c_t(t)
  lambda <- as.vector(contacts %*% (I / N)) * p_inf
  
  # aging rates: groups 1-8 age forward, group 9 loses to mortality
  a_vec <- c(a1, a2, a3, a4, a5, a6, a7, a8)
  a_out <- c(a_vec, mu_d)
  
  # inflows from aging (births into group 1; flow from previous group otherwise)
  ...
  # return derivatives interleaved as dS1,dE1,dI1,dR1,dS2,...,dR9
...
y0 <- c(S1 = tot1-5-floor(tot1*(imm_distribution(1, 3, 10))), E1 = 0, I1 = 5, R1 = floor(tot1*(imm_distribution(1, 3, 10))), # using random normal distribution for now, not informed by anything (need to change into something proper later)
        S2 = tot2-5-floor(tot2*(imm_distribution(2, 3, 10))), E2 = 0, I2 = 5, R2 = floor(tot2*(imm_distribution(2, 3, 10))),
        S3 = tot3-5-floor(tot3*(imm_distribution(3, 3, 10))), E3 = 0, I3 = 5, R3 = floor(tot3*(imm_distribution(3, 3, 10))),
        S4 = tot4-5-floor(tot4*(imm_distribution(4, 3, 10))), E4 = 0, I4 = 5, R4 = floor(tot4*(imm_distribution(4, 3, 10))),
        S5 = tot5-5-floor(tot5*(imm_distribution(5, 3, 10))), E5 = 0, I5 = 5, R5 = floor(tot5*(imm_distribution(5, 3, 10))),
        S6 = tot6-5-floor(tot6*(imm_distribution(6, 3, 10))), E6 = 0, I6 = 5, R6 = floor(tot6*(imm_distribution(6, 3, 10))),
        S7 = tot7-5-floor(tot7*(imm_distribution(7, 3, 10))), E7 = 0, I7 = 5, R7 = floor(tot7*(imm_distribution(7, 3, 10))),
        S8 = tot8-5-floor(tot8*(imm_distribution(8, 3, 10))), E8 = 0, I8 = 5, R8 = floor(tot8*(imm_distribution(8, 3, 10))),
        S9 = tot9-5-floor(tot9*(imm_distribution(9, 3, 10))), E9 = 0, I9 = 5, R9 = floor(tot9*(imm_distribution(9, 3, 10))))
# y0 <- c(S1 = tot1-5, E1 = 0, I1 = 5, R1 = 0, 
#         S2 = tot2-5, E2 = 0, I2 = 5, R2 = 0,
#         S3 = tot3-5, E3 = 0, I3 = 5, R3 = 0,
#         S4 = tot4-5, E4 = 0, I4 = 5, R4 = 0,
#         S5 = tot5-5, E5 = 0, I5 = 5, R5 = 0,
#         S6 = tot6-5, E6 = 0, I6 = 5, R6 = 0,
#         S7 = tot7-5, E7 = 0, I7 = 5, R7 = 0,
#         S8 = tot8-5, E8 = 0, I8 = 5, R8 = 0,
#         S9 = tot9-5, E9 = 0, I9 = 5, R9 = 0)
...
out <- out_parallel[[6]] # choose correct output for virus 
